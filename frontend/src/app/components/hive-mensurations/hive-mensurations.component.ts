...
import { Mensuration } from 'src/app/models/mensuration';
...
export class HiveMensurationsComponent implements AfterViewInit {
	...
	constructor(
		...
		private readonly _mensurationService: MensurationService,
	...
	buildGraph(mensurations: Array<Mensuration>) {
		...
		if (mensurations.length > 0) {
			...
			mensurations = mensurations.sort(
				(a, b) => new Date(a.createdDate).getTime() - new Date(b.createdDate).getTime()
			);

			this.lineChartDataTemperature.datasets.forEach(
				(x) => (x.data = mensurations.map((mensuration) => mensuration.temperature))
			);

			this.lineChartDataTemperature.labels = mensurations.map((mensuration) =>
				new Date(mensuration.createdDate).toLocaleString()
			);

			this.lineChartDataHumidity.datasets.forEach(
				(x) => (x.data = mensurations.map((mensuration) => mensuration.humidity))
			);

			this.lineChartDataHumidity.labels = mensurations.map((mensuration) =>
				new Date(mensuration.createdDate).toLocaleString()
			);

			this.lineChartDataWeight.datasets.forEach(
				(x) => (x.data = mensurations.map((mensuration) => mensuration.weight))
			);

			this.lineChartDataWeight.labels = mensurations.map((mensuration) =>
				new Date(mensuration.createdDate).toLocaleString()
			);
	...
	async search() {
		...
		this._mensurationService
			.search(this.hive.id, value, page, size, sort, direction)
			.subscribe({
				...
				error: (error) => {
					...
					this._notificationService.show(
						MessageUtils.MENSURATIONS_GET_FAIL,
